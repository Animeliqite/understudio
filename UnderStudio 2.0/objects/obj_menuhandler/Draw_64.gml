...
switch (state) {
	case 0:
		...
		draw_ftext(170 * sx, 41 * sy, dx_getraw("menu_screen", "menu_title"), fnt_main, c_silver, 1, 1, 1, 0, fa_left, fa_top);
		draw_rpgtext(170 * sx, 102 * sy, (gpConnected ? "`sprite:button_z`" : "[Z " + string_lower(dx_getraw("menu_screen", "menu_or")) + " ENTER]") + dx_getraw("menu_screen", "menu_confirm"), fnt_main, 1, -1, -1, 1, 1, c_silver);
		draw_rpgtext(170 * sx, 137 * sy, (gpConnected ? "`sprite:button_x`" : "[X " + string_lower(dx_getraw("menu_screen", "menu_or")) + " SHIFT]") + dx_getraw("menu_screen", "menu_cancel"), fnt_main, 1, -1, -1, 1, 1, c_silver);
		draw_rpgtext(170 * sx, 172 * sy, (gpConnected ? "`sprite:button_c`" : "[C " + string_lower(dx_getraw("menu_screen", "menu_or")) + " CTRL]") + dx_getraw("menu_screen", "menu_menu"), fnt_main, 1, -1, -1, 1, 1, c_silver);
		draw_rpgtext(170 * sx, 207 * sy, dx_getraw("menu_screen", "menu_fullscreen"), fnt_main, 1, -1, -1, 1, 1, c_silver);
		draw_rpgtext(170 * sx, 242 * sy, dx_getraw("menu_screen", "menu_quit"), fnt_main, 1, -1, -1, 1, 1, c_silver);
		draw_rpgtext(170 * sx, 277 * sy, dx_getraw("menu_screen", "menu_hp"), fnt_main, 1, -1, -1, 1, 1, c_silver);
		
		// Draw the selectable texts
		draw_rpgtext(170 * sx, 345 * sy, dx_getraw("menu_screen", "menu_begin"), fnt_main, 1, -1, -1, 1, 1, selection == 0 ? c_yellow : c_white);
		draw_rpgtext(170 * sx, 385 * sy, dx_getraw("menu_screen", "menu_settings"), fnt_main, 1, -1, -1, 1, 1, selection == 1 ? c_yellow : c_white);
