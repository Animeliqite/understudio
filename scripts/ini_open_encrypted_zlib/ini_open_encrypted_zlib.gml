/// @description ini_open_encrypted_zlib(input_file, encryption_key_string);
/// @function ini_open_encrypted_zlib
/// @param0 input_file
/// @param1 encryption_key_string
function ini_open_encrypted_zlib(argument0, argument1) {
	...
	******** README *****************************************************
	...
	2) If you are Writing or both Reading & Writing to/from .ini file, please use 'ini_close_encrypted_zlib' once you are finished.
	Script 'ini_close_encrypted_zlib' will compress and encrypt all data in the computer's memory and will save to disk already encrypted file without creating the unencrypted copy.

	******** USAGE EXAMPLE: *********************************************

	* Only READING from encrypted & compressed savedata.ini:

	ini_open_encrypted_zlib("savedata.ini","SomeEncryptionKey"); // Decrypt and open .ini
	...
	* READING & WRITING from/to encrypted & compressed savedata.ini:
	...
	ini_close_encrypted_zlib("savedata.ini","SomeEncryptionKey"); // Encrypt and save .ini
	...
	*/
	...
	if file_exists(_filename_in)
	{
		...
		// If we are decrypting & decompressing
		...
		{
			buffer_seek(_output_buffer, buffer_seek_start, 0); // Setting 'seek' at the initial position
			var string_from_buffer = buffer_read(_output_buffer, buffer_string); // Writing final decrypted & decompressed 'output_buffer' data to string
			ini_open_from_string(string_from_buffer);
